...
# Prepare packages
list.of.packages <- c("tidyverse", "rjags","jagsUI", "coda","parallel","doParallel", "foreach", "MCMCvis", "ggridges", "jagshelper", "GGally", "patchwork", "gridExtra", "ggpubr", "ggimage", "ggtext", "ggdist")
...
jf.wing <- readRDS(file = "data/MALL_juv_female_wing_combined.rda")
female.wing <- readRDS(file = "data/MALL_all_female_wing_combined.rda")

# jf.wing <- readRDS(file = "data/AGWT_juv_female_wing_combined.rda")
# female.wing <- readRDS(file = "data/AGWT_all_female_wing_combined.rda")
...
inits <- function(){list(f.am=runif(nyrs,0,0.5),f.af=runif(nyrs,0,0.5),f.jm=runif(nyrs,0,0.5),f.jf=runif(nyrs,0,0.5)
                        ...
                        # initial pop for mallard
                           # , n_HY_mal = runif(1, 200, 250), n_AHY_mal = runif(1, 200, 250), n_HY_fem = runif(1, 200, 250), n_AHY_fem = runif(1, 200, 250)
...
mall.h.ph <- data.frame(cbind(Year = rep(c(2000:2020), length(classes)*length(seasons)), 
                              Class = rep(rep(classes, each = length(mall.ipm$mean$R)), 2), 
                              Season = rep(seasons, each = 84), 
...
agwt.h.ph <- data.frame(cbind(Year = rep(c(2000:2020), length(classes)*length(seasons)), 
                              Class = rep(rep(classes, each = length(mall.ipm$mean$R)), 2), 
                              Season = rep(seasons, each = 84), 
...
S.a.mall.out <- data.frame(cbind(Year = rep(c(2000:2019),4), 
                 Class = rep(classes, each = length(2000:2019)), 
...
ggsave("figures/mall_annual_survival_2000_2019.jpg", units="cm", width = 10, height = 10, dpi = 600)

# AGWT
S.a.agwt.out <- data.frame(cbind(Class = rep(classes, each= length(2000:2019)), Year = rep(2000:2019, 4), S_a.mean = c(agwt.ipm$mean$S.jm, agwt.ipm$mean$S.jf, agwt.ipm$mean$S.am, agwt.ipm$mean$S.af), S_a.lower = c(agwt.ipm$q2.5$S.jm, agwt.ipm$q2.5$S.jf,agwt.ipm$q2.5$S.am, agwt.ipm$q2.5$S.af), S_a.upper = c(agwt.ipm$q97.5$S.jm, agwt.ipm$q97.5$S.jf, agwt.ipm$q97.5$S.am, agwt.ipm$q97.5$S.af)))
...
# MALL
mall.f.out <- data.frame(cbind(Class = rep(classes, each = length(2000:2020)), Year = rep(2000:2020, 4), f.mean = c(mall.ipm$mean$f.jm, mall.ipm$mean$f.jf, mall.ipm$mean$f.am, mall.ipm$mean$f.af), f.lower = c(mall.ipm$q2.5$f.jm, mall.ipm$q2.5$f.jf, mall.ipm$q2.5$f.am, mall.ipm$q2.5$f.af), f.upper = c(mall.ipm$q97.5$f.jm, mall.ipm$q97.5$f.jf, mall.ipm$q97.5$f.am, mall.ipm$q97.5$f.af)))
...
# AGWT
agwt.f.out <- data.frame(cbind(Class = rep(classes, each = length(2000:2020)), Year = rep(2000:2020, 4), f.mean = c(agwt.ipm$mean$f.jm, agwt.ipm$mean$f.jf, agwt.ipm$mean$f.am, agwt.ipm$mean$f.af), f.lower = c(agwt.ipm$q2.5$f.jm, agwt.ipm$q2.5$f.jf, agwt.ipm$q2.5$f.am, agwt.ipm$q2.5$f.af), f.upper = c(agwt.ipm$q97.5$f.jm, agwt.ipm$q97.5$f.jf, agwt.ipm$q97.5$f.am, agwt.ipm$q97.5$f.af)))

agwt.f.out <- agwt.f.out %>% 
  mutate_at(c('f.mean', 'f.lower', 'f.upper'), as.numeric)


agwt.recov.plot <- ggplot(agwt.f.out, aes(x = Year, y = f.mean)) +
  geom_pointrange(aes(ymin = f.lower, ymax = f.upper, group = Class,color = Class), size = 0.01, alpha = 0.7) +
...
theme_recruit <- function(){ theme(
  axis.text.x = element_text(size = 5, angle = 50, vjust = 0.7, hjust = 0.4),
  axis.text.y = element_text(size = 5),
...
# Plot
(R.plot <- ggplot(R.out, aes(x = Year, y = R.mean)) +
  geom_line(aes(group = Species, color = Species), size = 0.5) +
...
ggsave("figures/vulnerability_both.jpg", units="cm", width=8, height=7, dpi=600)

# overall population (bpop)
Bpop.both <- data.frame(cbind(Year = 2000:2020, Species = c(rep(species, each = length(agwt.ipm$mean$N_tot))), mean = c(mall.ipm$mean$N_tot, agwt.ipm$mean$N_tot), lower = c(mall.ipm$q2.5$N_tot, agwt.ipm$q2.5$N_tot), upper = c(mall.ipm$q97.5$N_tot, agwt.ipm$q97.5$N_tot)))
...
ggsave("figures/Bpop_size_both.jpg", units="cm", width=8, height=7, dpi=600)

# Plotting population by age-sex class
mall.Bpop.out <- data.frame(cbind(Class = rep(c("Juvenile Male", "Juvenile Female", "Adult Male", "Adult Female"), each = length(mall.ipm$mean$N_AHY_fem)), Year = rep(2000:2020, 4), Bpop.mean = c(mall.ipm$mean$N_HY_mal, mall.ipm$mean$N_HY_fem, mall.ipm$mean$N_AHY_mal, mall.ipm$mean$N_AHY_fem), Bpop.lower = c(mall.ipm$q2.5$N_HY_mal, mall.ipm$q2.5$N_HY_fem, mall.ipm$q2.5$N_AHY_mal, mall.ipm$q2.5$N_AHY_fem), Bpop.upper = c(mall.ipm$q97.5$N_HY_mal, mall.ipm$q97.5$N_HY_fem, mall.ipm$q97.5$N_AHY_mal, mall.ipm$q97.5$N_AHY_fem)))
...
agwt.Bpop.plot <- ggplot(agwt.Bpop.out, aes(x = Year, y = Bpop.mean, group = Class)) +
  ...
  geom_ribbon(aes(ymin = Bpop.lower, ymax = Bpop.upper, fill = Class ), alpha = 0.5) +
  theme(axis.text.x = element_text(angle = 45)) +
  ggtitle('Green-winged teal population by size-class')
...
(alpha.plot <- ggplot(alpha.all, aes(x = Covariate_effect, y = Covariate)) +
...
ggsave("figures/mall_cov_effects_surv_2000_2020.jpg", units="cm", width=10, height=7, dpi=600)
...
(mall.beta.plot <- ggplot(mall.beta.all, aes(x = Covariate_effect, y = Covariate)) +
  ...
  xlim(-0.3, 0.3) +
  annotate("text", x=-0.28, y=3.75, label= "(a)") +
  annotate("text", x=0.28, y=3, label= "*") +
    annotate("text", x=0.28, y=2, label= "*") +
...
(agwt.alpha.plot <- ggplot(agwt.alpha.all, aes(x = Covariate_effect, y = Covariate)) +
  stat_halfeye(aes(fill = Class), interval_size_range = c(0.25, 0.45), position = "dodge")+
  theme_cov() +
  geom_vline(xintercept = 0, linetype="dashed", color = "black", linewidth=0.3) +
  scale_fill_manual(values=cbbPalette) +
...
ggsave("figures/agwt_cov_effects_surv_2000_2020.jpg", units = "cm", width = 10, height = 7, dpi = 600)
...
for(i in 1:dims[3]){
  for(j in 1:dims[4]){
    totRecov[,i,j] <- rowSums(MALL.marray[, 1:dims[1], i, j])
  }
}
...
totRelease <- apply(MALL.release, 2, identity)
