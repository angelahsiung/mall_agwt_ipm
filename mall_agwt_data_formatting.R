...
clean[recov$R.Month<2, 3]  <- recov[recov$R.Month<2, "R.Year"]-1 # recoveries in January belong in the hunting season of previous year

# Bring in age
clean[,4] <- NA
clean[recov$Age..VAGE=="Local"|recov$Age..VAGE=="Hatch Year", 4] <- 1 # 1 indicates juvenile
clean[recov$Age..VAGE=="After Hatch Year" & recov$B.Month%in%c(2:3), 4] <- 1 # AHY banded post-season is considered juveile
...
marray <- array(NA, dim = c(nYear, nYear, nSeason, nClass),
                dimnames = list(Year, Year, c("post-season", "pre-season"),
...
bands <- array(NA, dim = c(nYear, 1, nSeason, nClass),
                    dimnames = list(Year, "Banded", c("post-season", "pre-season"), 
                    c("Juvenile_Male", "Juvenile_Female", "Adult_Male", "Adult_Female")))
...
nonrecov <- array(NA, dim = c(nYear, 1, nSeason, nClass),
...
MALL.marray<-array(NA,dim=c(nYear,nYear+1,nSeason,nClass),
                  dimnames =list(Year, c(Year,"Nonrecov"), c("post-season", "pre-season"),
                                 c("Juvenile_Male", "Juvenile_Female", "Adult_Male", "Adult_Female")))
MALL.release<-array(NA,dim=c(nYear,nSeason,nClass),
...
mall.wings <- read.csv("PCS_MALL_1991_2020.csv")
#agwt.wings <- read.csv("PCS_AGWT_1991_2020.csv")

# Remove unknown age or sex
mall.wings <- mall.wings[!(mall.wings$age_code==0 | mall.wings$sex_code==0),] # 889384 records remaining
# agwt.wings <- mall.wings[!(mall.wings$age_code==0 | mall.wings$sex_code==0),] # 889384 records remaining
...
saveRDS(jf.wing, file = "mall_juv_female_wing.rda")
saveRDS(female.wing, file = "mall_all_female_wing.rda")
